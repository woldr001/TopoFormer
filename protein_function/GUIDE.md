...
## 6. Running the NMA-PCA Pipeline

This step computes the GNM/PCA motion profiles that drive residue selection in the topology extractor.
The NMA-PCA script (`protein_function/scripts/run_nma_pca.py`) is built into this repository and
uses ProDy directly — no external pipeline repo is required.
...
python protein_function/scripts/run_nma_pca.py \
    --pdb_dir      /data/pdbs/P12345/clustering.nodes_pipeline_v1 \
...
```
anm_gnm_results.npz   ← keys: 'gnm_flucts_mean' [n_slow_modes, N_residues], 'resids' [N_residues]
pca_results.npz       ← keys: 'pc1_profile' [N_residues], 'pc2_profile' [N_residues], 'resids'
```

### 6.3 Batch processing via SLURM array job

Use the provided launcher, which chunks proteins to stay within SLURM QOS limits (5 proteins
per task → ~489 tasks for 2,441 proteins):

```bash
# Creates all_ids.txt from train/val/test splits, then submits the array job
bash protein_function/scripts/submit_nma_pca.sh
```

The array job script (`sbatch_nma_pca.sh`) processes each chunk sequentially with warnings-only
error handling so one failed protein does not abort the task.

### 6.4 QA check after completion

```python
import os, numpy as np
nma_dir = '/data/nma_pca'
ok, missing = 0, []
for pid in os.listdir(nma_dir):
    d = os.path.join(nma_dir, pid)
    if os.path.exists(os.path.join(d, 'anm_gnm_results.npz')) and \
       os.path.exists(os.path.join(d, 'pca_results.npz')):
        ok += 1
    else:
        missing.append(pid)
print(f'OK: {ok} | missing both files: {len(missing)}')
```

### 6.5 What if NMA-PCA files are missing?

The topology extractor falls back to **treating all residues as motion residues** if the `.npz`
files are absent — it will warn and proceed. The model will still train, but the motion-guided
signal will be absent and the topology reduces to a standard intra-protein scheme.
...
The aSAM pattern is checked first, so no configuration is needed for data generated by the
aSAM pipeline at `/mnt/research/nodes/giacomo/asam_ensembles/`.

### 7.5 Expected runtime

Topology extraction is the most compute-intensive step. Runtime per protein depends heavily on
sequence length (eigenvalue computation scales as O(N³) in atom count):

| Protein size | Approx. time (1 CPU core) |
|-------------|--------------------------|
| Short (≤150 aa) | 2–10 min |
| Medium (150–400 aa) | 10–60 min |
| Long (>400 aa) | 1–4+ hours |

**Use the SLURM array job** (`submit_topo_features.sh`) rather than a single-node job.
With 489 tasks across cluster nodes, 2,441 proteins complete in a few hours.

### 7.6 SLURM array job (recommended for large datasets)

```bash
# Submits ~489 tasks (CHUNK_SIZE=5 proteins per task, 5 CPUs each)
...
### 7.7 Fit and save the topology scaler
...
└── protein_function/                  ← protein function prediction code
    ...
    ├── README.md                      ← MSU HPCC step-by-step guide
    ...
    └── scripts/
        ...
        ├── filter_pdb_by_length.py           ← keep proteins ≤500 aa
        ├── run_nma_pca.py                    ← ProDy GNM+PCA for one protein
        ├── submit_nma_pca.sh                 ← SLURM array job launcher (NMA-PCA)
        ├── sbatch_nma_pca.sh                 ← SLURM array worker (NMA-PCA)
        ├── report_motion_residues.py         ← motion/static residue count report
        ├── precompute_topo_features.py       ← batch topology extraction
        ├── submit_topo_features.sh           ← SLURM array job launcher (topology)
        ├── sbatch_topo_features_array.sh     ← SLURM array worker (topology)
        ├── sbatch_topo_features.sh           ← single-node topology job (small runs)
...
- [ ] NMA-PCA run via `bash protein_function/scripts/submit_nma_pca.sh`; `anm_gnm_results.npz` + `pca_results.npz` ready per protein
- [ ] Topology features pre-computed via `bash protein_function/scripts/submit_topo_features.sh` (SLURM array job)
...
*Guide written for branch `protein-motion-topology`. For questions or issues, open a GitHub issue on `woldr001/TopoFormer`.*
